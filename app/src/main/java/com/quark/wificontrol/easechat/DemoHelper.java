...
import com.quark.api.auto.bean.Avatars;
import com.quark.api.auto.bean.AvatarsRequest;
import com.quark.api.auto.bean.AvatarsResponse;
...
import com.quark.wificontrol.easechat.db.DemoDBManager;
import com.quark.wificontrol.easechat.db.InviteMessgeDao;
import com.quark.wificontrol.easechat.db.UserDao;
import com.quark.wificontrol.easechat.domain.InviteMessage;
import com.quark.wificontrol.easechat.domain.InviteMessage.InviteMesageStatus;
import com.quark.wificontrol.easechat.domain.RobotUser;
import com.quark.wificontrol.easechat.parse.UserProfileManager;
import com.quark.wificontrol.easechat.receiver.CallReceiver;
...
public class DemoHelper {
    ...
    //=====如果你想easeui处理头像和昵称设置配置文件提供程序  leon===========
...
//        });
        ...
        easeUI.getNotifier().setNotificationInfoProvider(new EaseNotificationInfoProvider() {
            ...
            public String getTitle(EMMessage message) {
                //you can update title here  leon
                return null;
            }

            @Override
            public int getSmallIcon(EMMessage message) {
                //you can update icon here leon
                return 0;
            }
...
//                }
    ...
    private EaseUser getUserInfo(String username) {
        ...
        if (user == null) {
            ...
            getOneUserAvatars(user); //leon  如果没有在数据库 插入到数据库中
        }
//        user.setNickname("00xx");
//        user.setAvatar("");
...
//                    });
    ...
    //==============================从服务器获取头像昵称============================
    public void getOneUserAvatars(EaseUser user) {
        usertemp = user;
        AvatarsRequest rq = new AvatarsRequest();
        rq.setTelephones("{" + user.getUsername() + "}");
        QuarkApi.HttpRequest(rq, mHandlerOne);
    }

    private final AsyncHttpResponseHandler mHandlerOne = new AsyncHttpResponseHandler() {
        @Override
        public void onSuccess(int arg0, Header[] arg1, byte[] arg2) {
            Object kd = ApiResponse.get(arg2, AppContext.instance, AvatarsResponse.class);
            if (kd != null) {
                AvatarsResponse info = (AvatarsResponse) kd;
                List<Avatars> avatars = info.getAvatars();
                UserDao dao = new UserDao(appContext);
                EaseUser users = usertemp;
                users.setAvatar(ApiHttpClient.loadImage + avatars.get(0).getAvatar());
                users.setNickname(avatars.get(0).getName());
                dao.saveContact(users);
...
//                notifyContactsSyncListener(true);
    ...
    public void getUserAvatars(final Map<String, EaseUser> userlisty) {
        userlist = userlisty;
//        List<EaseUser> userstmep = new ArrayList<EaseUser>(userlist.values());
        String telephones = "";
        for (Map.Entry<String, EaseUser> entry : userlist.entrySet()) {
            telephones += entry.getKey() + "、";
            System.out.println("key= " + entry.getKey() + " and value= ");
        }
        if (telephones.endsWith("、")) {
            telephones = telephones.substring(0, telephones.length() - 1);
        }

        AvatarsRequest rq = new AvatarsRequest();
        rq.setTelephones("{" + telephones + "}");
...
//                        users.get(i).setAvatar(avatars.get(i).getAvatar());
//                        users.get(i).setNickname(avatars.get(i).getName());
//                    }
//                    dao.saveContactList(users);
//
//                    demoModel.setContactSynced(true);
    ...
    private final AsyncHttpResponseHandler mHandler = new AsyncHttpResponseHandler() {
        ...
        public void onSuccess(int arg0, Header[] arg1, byte[] arg2) {
            ...
            if (kd != null) {
                ...
                List<EaseUser> users = new ArrayList<EaseUser>(userlist.values());
                for (int i = 0, size = users.size(); i < size; i++) {
                    users.get(i).setAvatar(ApiHttpClient.loadImage + avatars.get(i).getAvatar());
                    users.get(i).setNickname(avatars.get(i).getName());
                }
                dao.saveContactList(users);
